...
- Confirmed content now covers the association introduction, three Chinese-titled learning directions, a two-node origin story, the rough first-year training route, and 21 partial alumni journeys across the 21, 22, and 23 cohorts. Detailed intermediate historical milestones are still pending.
- The outcome section is now one `Alumni Journey`, not a split achievements/alumni panel. Implementation is underway after the user's approval.
...
- The user supplied 21 partial alumni records: 6 from the 21 cohort, 6 from the 22 cohort, and 9 internship journeys from the 23 cohort.
...
- Dark mode must visibly use Klein blue for headings, controls, lines, and editorial details so red does not dominate; red remains reserved for the logo A and sparse semantic nodes.
...
- The user supplied eight PNG destination decorations. Use them as small, aspect-ratio-preserving local images; remove corresponding company SVG animation components.
- The alumni route should read as a 2.5D tree timeline: STA as root, cohort nodes as trunk, and left/right alumni branches as leaves. CSS perspective is allowed; heavy 3D/WebGL is not.
...
- Desktop uses a sticky 21/22/23 cohort index, central trunk, left/right branches, and restrained CSS perspective. Mobile uses a conventional vertical trunk without scroll hijacking; reduced motion shows final states immediately.
- The supplied company-icon document is superseded for runtime use by eight user-supplied PNG decorations. Keep the images local, small, and static; company text remains primary.
...
- Avoid full 3D globes, random particle fields, neon glow, heavy glassmorphism, and multiple competing ambient animations. A restrained CSS 2.5D tree is now approved.
...
- Alumni destination images: `src/assets/alumni-icons/` (eight user-supplied PNGs)
...
- The supplied company-icon animation guide is not used at runtime. The eight supplied PNGs cover the company destinations in the provided records; the two university destinations remain text-only.
...
4. Complete browser-level mobile QA of the 2.5D tree when a device-size viewport is available.
